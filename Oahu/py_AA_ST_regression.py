...
def AAA_ST_regress(data_orig, bounds, flag_df_one_add_one):
    ...
    Cd_inv_orig = inv(Cd_orig) # invert the square matrix. (* basically 1/x_i where x_i is each diagonal value)
    # Cd_inv_orig = inv(Cd_orig) @ np.eye(Cd_orig.shape[0])
    ...
    ST_Cm_sm = ST_Cm.copy()
    ST_Cm_sm = sp.lil_matrix(ST_Cm_sm)
    ...
    # Plot data in FEET
    fig101, ax101 = plt.subplots(figsize=(10,6))
    # ax101.grid(True)
    ...
    fig1001, axs = plt.subplots(2, 1, figsize=(12, 6))
    axs[0].grid(True, lw=0.4)
    axs[1].grid(True, lw=0.4)
    ...
    #subset ST to save rates as CSV
    w = ['rate', 'rate_var', 'rate_sm', 'rate_var_sm', 'int', 'int_var']
    ST_subset_dict = dict((k, ST[k]) for k in w if k in ST)
    ST_subset = pd.DataFrame.from_dict(ST_subset_dict, orient='index')


    return ST, figs_out, ST_subset
