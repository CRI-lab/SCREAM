...
full_original = np.copy(full) # keep a copy
...
truncated_data = np.full_like(full, np.nan)
# put hard shoreline values in matrix for plotting later
hardshore_data = np.full_like(full, np.nan)
hardshore_data_flat = np.full_like(full, np.nan)
hardshore_data_flat_truncated = np.full_like(full, np.nan)
...
for trans_start, trans_end, col_start, col_end in trunc:
    ...
    full[row_start:row_end, col_end:] = np.nan #if assuming third column is always 1, this is same as data_trunc #DATA_TRUNC.. #full[row_start:row_end, ~keep_cols] = np.nan
...
bools_full = ~np.isclose(full, full_original, equal_nan=True) #tracks the changed values between the two data arrays after the truncations. True if changed
bools_full_flat =  ~np.isclose(full_flat, full_original, equal_nan=True)
# truncated_data = np.copy(full).astype(float)
# truncated_data[~bools_full] = np.nan
# hardshore_data_flat = np.copy(full).astype(float);
